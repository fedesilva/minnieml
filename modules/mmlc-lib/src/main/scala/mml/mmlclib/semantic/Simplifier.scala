...
object Simplifier:
      ...
      case single :: Nil =>
        ...
        single match
          case e: Expr =>
            ...
            e
          case appN: AppN =>
            // Simplify AppN arguments
            val simplifiedArgs = appN.args.map(simplifyExpr)
            Expr(expr.span, List(appN.copy(args = simplifiedArgs)))
          case _ =>
  ...
  def simplifyTerm(term: Term): Term =
    term match
      ...
      case appN: AppN =>
        // Simplify the function arguments
        val simplifiedArgs = appN.args.map(simplifyExpr)
        appN.copy(args = simplifiedArgs)
