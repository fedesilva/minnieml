...
object DuplicateNameChecker:
  ...
  /** Rewrite module to replace duplicate members with invalid nodes, accumulating errors in the state. */
  ...
  private def processMembers(members: List[Member]): (List[Member], List[SemanticError]) =
    ...
    // First, handle duplicate member names
    val membersByKey = groupMembersByKey(members)
    val membersAfterDuplicates = membersByKey.flatMap { case (key, items) =>
      if items.size > 1 then
        // Report error
        errors += SemanticError.DuplicateName(key._1, items.collect { case d: Decl => d }, phaseName)
    ...
    val finalMembers = membersAfterDuplicates.map {
      case fn: FnDef if hasDuplicateParams(fn.params) =>
        val duplicateParamNames = fn.params.groupBy(_.name).filter(_._2.size > 1).keys.mkString(", ")
    ...
    val operatorNames = members.collect { case op: OpDef => op.name }.toSet
    members.collect {
      case fn: FnDef if operatorNames.contains(fn.name) =>
        val allWithSameName = members.filter {
    ...
    (finalMembers, errors.toList)

  private def groupMembersByKey(members: List[Member]): Map[(String, String), List[Member]] =
    members.foldLeft(Map.empty[(String, String), List[Member]]) { (acc, member) =>
      member match
        case d: Decl =>
