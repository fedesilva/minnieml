...
object RefResolver:
  ...
  /** Resolve references in an expression.
    *
    * We traverse the list of terms while tracking whether we expect an operand. In operand position
    * (e.g. at the beginning, after an operator, or inside a group), we prefer to resolve to a unary
    * operator. In operator position, we prefer a binary operator.
    */
  private def resolveExpr(
    ...
    // The state is a triple: (accumulated terms, flag indicating if an operand is expected, accumulated errors).
    // Initially, we expect an operand.
        ...
        term match
          case ref: Ref =>
            ...
            if expectOperand then
              // In operand position: look for a unary operator.
              candidates.collect { case op: UnaryOpDef => op } match
                case unaryOp :: _ =>
                  (acc :+ ref.copy(resolvedAs = Some(unaryOp)), false, errs)
                case Nil =>
                  // If no unary op is found, but a binary op exists, flag an error.
              ...
              // In operator position: expect a binary operator.
              candidates.collect { case op: BinOpDef => op } match
                case binOp :: _ =>
                  (acc :+ ref.copy(resolvedAs = Some(binOp)), true, errs)
                case Nil =>
                  // No binary candidate found. If a unary op exists here, that's not valid.
                  candidates.collect { case op: UnaryOpDef => op } match
                    case unaryOp :: _ =>
                      (
                        acc :+ ref.copy(resolvedAs = Some(unaryOp)),
                        true,
                        errs :+ SemanticError.UndefinedRef(ref, owner)
                      )
                    case Nil =>
                      (acc :+ ref, true, errs :+ SemanticError.UndefinedRef(ref, owner))
