...
/** Descriptor for native LLVM operations that can be referenced by user-defined operators.
  *
  * This provides a mapping from user-specified selectors (e.g., "add", "sub") to the actual
  * LLVM instruction templates. The codegen uses this to emit proper LLVM IR without hardcoded types.
  *
  * @param selector
...
object NativeOpRegistry:
  ...
  /** Look up a binary operation descriptor by selector.
    *
    * @param selector the operation selector from @native[op=selector]
    * @return Some(descriptor) if found, None otherwise
    */
