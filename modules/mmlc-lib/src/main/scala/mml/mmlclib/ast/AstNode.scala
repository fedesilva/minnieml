...
/** Marker trait for nodes that represent invalid/error constructs.
  * These nodes allow the compiler to continue processing even when errors are encountered,
  * enabling better LSP support and partial compilation.
  */
sealed trait InvalidNode extends AstNode

/** Represents an expression that could not be resolved or is otherwise invalid.
  * Preserves the original expression for debugging and error reporting.
  */
...
/** Represents a type specification that could not be resolved.
  * Preserves the original type for debugging and error reporting.
  */
...
/** Represents a duplicate member declaration.
  * The first occurrence remains valid and referenceable, subsequent duplicates
  * are wrapped in this node.
  */
...
/** Represents a member that is invalid for reasons other than being a duplicate.
  * For example, functions with duplicate parameter names.
  */
