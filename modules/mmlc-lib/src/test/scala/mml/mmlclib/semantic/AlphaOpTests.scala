      ...
      bnd match
        case bnd: Bnd =>
          ...
          bnd.value.terms match
            case List(TXApp(orRef, _, List(xorExpr, dExpr))) 
                if orRef.name == "or" =>
              ...
              xorExpr.terms match
                case List(TXApp(xorRef, _, List(andExpr, cExpr))) 
                    if xorRef.name == "xor" =>
                  ...
                  andExpr.terms match
                    case List(TXApp(andRef, _, List(aExpr, bExpr))) 
                        if andRef.name == "and" =>
            ...
            case other =>
              fail(s"Expected (((a and b) xor c) or d) app structure, got: ${other.map(term => prettyPrintAst(term)).mkString(", ")}")
    ...
    semNotFailed(
      ...
      bnd match
        case bnd: Bnd =>
          ...
          bnd.value.terms match
            case List(TXApp(andRef, _, List(negateExpr, notExpr))) 
                if andRef.name == "and" =>
              
              // Check left: negate a
              negateExpr.terms match
                case List(TXApp(negateRef, _, List(aExpr))) 
                    if negateRef.name == "negate" =>
              ...
              // Check right: not b
              notExpr.terms match
                case List(TXApp(notRef, _, List(bExpr))) 
                    if notRef.name == "not" =>
            ...
            case other =>
              fail(s"Expected '(negate a) and (not b)' app structure, got: ${other.map(term => prettyPrintAst(term)).mkString(", ")}")
    ...
    semNotFailed(
      ...
      bnd match
        case bnd: Bnd =>
          ...
          bnd.value.terms match
            case List(TXApp(impliesRef, _, List(aExpr, bExpr)))
                if impliesRef.name == "implies" =>
    ...
    semNotFailed(
      ...
      bnd match
        case bnd: Bnd =>
          ...
          // Verify expression structure
          bnd.value.terms match
            case List(TXApp(unlessRef, _, List(aExpr, bExpr)))
                if unlessRef.name == "unless" =>
    ...
    semNotFailed(
      ...
      bnd match
        case bnd: Bnd =>
          ...
          bnd.value.terms match
            case List(TXApp(xorRef, _, List(notExpr, andExpr)))
                if xorRef.name == "xor" =>
              
              // Check left: not a
              notExpr.terms match
                case List(TXApp(notRef, _, List(aExpr)))
                    if notRef.name == "not" =>
              ...
              andExpr.terms match
                case List(TXApp(andRef, _, List(negateExpr, cExpr)))
                    if andRef.name == "and" =>
                  
                  // Check negate b
                  negateExpr.terms match
                    case List(TXApp(negateRef, _, List(bExpr)))
                        if negateRef.name == "negate" =>
            ...
            case other =>
              fail(s"Expected structure '(not a) xor ((negate b) and c)', got: ${other.map(term => prettyPrintAst(term)).mkString(", ")}")
    ...
    semNotFailed(
      ...
      bnd match
        case bnd: Bnd =>
          ...
          bnd.value.terms match
            case List(TXApp(outerOrRef, _, List(innerOrExpr, cExpr)))
                if outerOrRef.name == "or" =>
              ...
              innerOrExpr.terms match
                case List(TXApp(innerOrRef, _, List(notAExpr1, andExpr)))
                  ...
                  notAExpr1.terms match
                    case List(TXApp(notRef1, _, List(aExpr1)))
                        if notRef1.name == "not" =>
                  ...
                  andExpr.terms match
                    case List(TXApp(andRef, _, List(notBExpr, notAExpr2)))
                      ...
                      notBExpr.terms match
                        case List(TXApp(notRef2, _, List(bExpr)))
                            if notRef2.name == "not" =>
                        ...
                        case other =>
                          fail(s"Expected 'not b' in the and expression, got: ${prettyPrintAst(notBExpr)}")
                      
                      // Check not a (right side of and)
                      notAExpr2.terms match
                        case List(TXApp(notRef3, _, List(aExpr2)))
                            if notRef3.name == "not" =>
                        ...
                        case other =>
                          fail(s"Expected 'not a' in the and expression, got: ${prettyPrintAst(notAExpr2)}")
                    
                    case other =>
                      fail(s"Expected 'not b and not a' expression, got: ${prettyPrintAst(andExpr)}")
                
                case other =>
                  fail(s"Expected inner or expression, got: ${prettyPrintAst(innerOrExpr)}")
            
            case other =>
              fail(s"Expected top-level or expression, got: ${other.map(term => prettyPrintAst(term)).mkString(", ")}")
