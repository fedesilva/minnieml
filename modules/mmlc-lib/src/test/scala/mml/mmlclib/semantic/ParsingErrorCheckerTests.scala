...
class ParsingErrorCheckerTests extends BaseEffFunSuite:
  ...
  // Pending migration: Parent compiler has no TermErrorFound diagnostic.
  test("MemberErrorChecker should catch term errors nested inside expressions".ignore) {
     ...
     */
    fail("Parent compiler has no TermErrorFound diagnostic.")
  }
