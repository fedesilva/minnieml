...
- `structKw` → `TypeStruct` + synthetic constructor `Bnd` (see below)
...
After parsing all members, `addStructConstructors` walks the member list and appends a
synthetic constructor `Bnd` after each `TypeStruct`. The constructor is generated by
`mkStructConstructor`:
...
all downstream phases (IdAssigner, TypeChecker, etc.) process it uniformly.
