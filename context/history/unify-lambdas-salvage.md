# Lambda restart provenance

Historical restart inventory and recovery instructions, retained for Author review. Current
progress and the standing handoff requirement live in [the lambda task](../tasks/unify-lambdas.md).
Original relative paths, branch state, and approvals below belong to their dated checkpoint.
They do not authorize new work. The readability assessment is now
[its own task](../tasks/qa-readability.md).
...
Resume on `dev-lambdas-migration`. Read [lambda-test-migration.md](lambda-test-migration.md) for the current state,
...
recoverable for the test history. No compiler rewrite or tracked-item completion is approved by this checkpoint.

## Current document locations

Current semantic goals and the PAP ownership contract are in
[the lambda task](tasks/unify-lambdas.md). The duplicate-capture report is in
[its task](tasks/lambda-lifting-bug.md). Source designs, prior implementation plans,
and reviews are archived in `context/history/` for Author review. The full source design
is [unify-lambdas-design.md](history/unify-lambdas-design.md); the other archived reports
retain their original basenames. The historical inventory below uses the original paths;
Git recovery commands must keep those paths to read the correct source objects.
