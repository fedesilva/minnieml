# Validate native templates in the check command
...
Review the existing ticket for the check command.

- Add a phase that runs `llvm-as` only when native declarations with templates are detected.
- The check command runs ingest, semantic analysis, and LLVM validation when required.
- This phase runs only during check; ordinary compilation uses the normal toolchain.

## Outcome

The check command validates native templates when required without duplicating the ordinary compilation toolchain.
