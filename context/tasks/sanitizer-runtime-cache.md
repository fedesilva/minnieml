# Separate sanitizer runtime cache artifacts
...
When compiling with ASan, compile the runtime with an `_asan` suffix and link to
that runtime. The runtime binary is cached; reusing the ASan runtime when compiling
without ASan causes linking to fail.

## Outcome

Sanitized and ordinary compilations select distinct cached runtime artifacts.
