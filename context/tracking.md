...
- 2026-03-16: QA Debt: OwnershipAnalyzerTests refactoring.
  - Extracted custom extractors (`Unwrapped`, `Call1`, `RefNamed`, `RefResolved`) replacing brittle wildcard-heavy AST pattern matches.
  - Centralized recursive tree traversal into generic `existsTerm`/`countTerm` walkers, eliminating duplication across 5 helper functions.
  - `containsFreeOf` and `containsFreeString` now check call position via `Call1`, matching resolved IDs when available.
  - All 35 ownership tests pass.

- 2026-03-15: Visibility enforcement and linkage.
  - `pub fn main` required in Exe mode; `PreCodegenValidator` emits `InvalidEntryPoint` if missing.
  - Non-pub user functions emit `internal` LLVM linkage; `pub` functions get external linkage.
  - Added `SemanticError.VisibilityViolation` variant for future cross-module enforcement.
  - Parser fix: added `pub`/`prot`/`priv` to `keywords` list to prevent `exprP` consuming across member boundaries.
  - `opt` pass uses `--passes='default<ON>,internalize,globaldce'` in Exe mode to internalize bitcode-linked runtime symbols.
  - Updated all `.mml` samples, benchmarks, and test harnesses (`pub fn main`).
  - New tests for visibility check and `internal` linkage in `PreCodegenValidatorSuite` and `FunctionSignatureTest`.

- 2026-03-14: Global borrow-only ownership semantics.
  - Added `OwnershipState.Global` to track top-level bindings.
  - Updated `OwnershipAnalyzer` to seed top-level heap-allocated bindings into the global scope.
  - Updated `argNeedsClone` to return `true` for global bindings.
  - Extended `handleConsumingParam` to accept `Global` bindings without moving them, preserving their valid state.
  - Added `@tailrec` `unwrapTerm` helper to ensure grouped globals (e.g. `(name)`) are still detected and cloned correctly.
  - Removed constructor-only guard for auto-cloning, so globals are cloned for any consuming param.
  - Added tests in `OwnershipAnalyzerTests.scala` and samples (`person-struct-borrow-global.mml`, `globals-stress-test.mml`).

- 2026-03-14: Fix TCO for nested conditional branches [COMPLETE].
  - Replaced flat `TailRecPattern` model with recursive `TailRecBody` tree
    (`TailRecExit`, `TailRecCall`, `TailRecBranch`) in `FunctionEmitter.scala`.
  - New `extractBody` walker (45 lines) replaces 5 extraction helpers (185 lines).
    Both-branches-recursive is now valid, not rejected.
  - New `compileTailRecBody` recursively emits LLVM IR, collecting `BackEdge` lists
    for multi-valued phi nodes. Every leaf terminates with `ret` or `br label %loop.header`.
  - Removed: `ExitBranch`, `TailRecPattern`, `compileExitBranchChecks`,
    `compileCompoundConditions`, `compileExitBlocks`, `extractCondChainPattern`,
    `extractRecursiveBranch`, `extractSelfCallWithPreStatements`,
    `extractNestedLatchCond`, `findTerminalCondWithPreStatements`.
  - Updated 4 existing tests, added 3 new tests (both-branches-recursive,
    nested different args, deep mixed exits/calls). 300/300 tests pass.
  - `astar2.mml` `visit_neighbors` now emits 4 back edges instead of falling
    back to standard calls. Net -168 lines.

- 2026-03-14: Synthetic local ID resolution for partial application and ownership wrappers [COMPLETE].
  - Added `SyntheticLocals` helper (`semantic/SyntheticLocals.scala`): creates synthetic
    `FnParam` + `Ref` pairs with immediate UUID-based nested ID assignment.
  - Updated `ExpressionRewriter` eta-expansion to use the helper for `$pN` params/refs —
    partial-app generated refs are now self-resolved at construction time.
  - Updated `OwnershipAnalyzer` wrappers to use the helper for `__tmp_N`, `__tmp_result`,
    `__ownership_result`, and discard `_` params.
  - Added `resolvables-indexer-final` pass at end of `SemanticStage` (after `ownership-analyzer`)
    so all synthetic IDs appear in `module.resolvables`.
  - Added `SyntheticLocalResolutionTests.scala` with regression tests asserting `resolvedId`,
    `candidateIds`, and `resolvables.lookup` for both eta-expansion and ownership temp wrappers.

- 2026-02-27: `@native[name="..."]` attribute support added [COMPLETE].
  - Added `nativeSymbol: Option[String] = None` to `NativeImpl` AST node (`ast/terms.scala`).
  - Refactored `nativeImplP` parser to `enum Attr` + `.rep(sep=",")` + `collectFirst` pattern,
    supporting `tpl`, `mem`, and `name` attrs in any order (`parser/expressions.scala`).
  - Codegen declaration emitter uses `nativeSymbol.getOrElse(fnName)` as declared C symbol
    (`codegen/emitter/Module.scala`).
  - Codegen call emitter resolves to native symbol via `getNativeSymbol` helper
    (`codegen/emitter/expression/package.scala`).
  - All pattern match sites updated; pretty-printer displays `[name="..."]`.
  - 6 new tests (4 parser, 2 codegen); all 293 tests pass.
...
### 2026-02-15 Memory edge-case testing marked complete
...
### 2026-02-14 User-struct clone resolution in ownership analyzer
...
### 2026-02-13 Conditional ownership merge + memory harness progress output
...
### 2026-02-10 Preserve `.5` float literals with `.` operator
...
### 2026-02-10 Parser whitespace regression for `=` and `;`
...
### 2026-02-10 Negative test harness complete
...
### 2026-02-09 Move constructor generation from parser to semantic phase
...
### 2026-02-09 Memory test harness
...
### 2026-02-09 Native struct constructor generation and field access
...
### 2026-02-09 Nested struct destructor resolution and ownership tracking
...
### 2026-02-09 OOM Policy B: trap on allocation failure
...
### 2026-02-09 OwnershipAnalyzer code quality cleanup
...
### 2026-02-08 Move-only structs + LSP ownership diagnostics
...
### 2026-02-08 Fix consuming param memory leaks + BindingOrigin metadata
...
### 2026-02-07 Borrow escape enforcement (Rule 1: return)
...
### 2026-02-07 Add `noalias` attributes to LLVM IR emission
...
### 2026-02-07 Last-use validation for consuming parameters
...
### 2026-02-07 Partial application ban for consuming parameters
...
### 2026-02-07 Fix `arrays-mem.mml` double-free
...
### 2026-02-06 Update language reference
...
### 2026-02-06 Fix struct semantic token miscoloring
...
### 2026-02-06 LSP crash fix + logging
...
### 2026-02-06 JsonRpc Header Parsing Fix
...
### 2026-02-06 Right-assoc operator use-after-free fix
...
### 2026-02-04 Phase 3: Struct memory function generation
...
### 2026-02-04 Struct constructor ownership tracking
