...
- [ ] **Use-after-free in chained user-function calls that wrap allocating natives**
  - **Repro:** `mml/samples/concat-op.mml` — `op ++(a,b) = concat a b` chained as
    `"Zero: " ++ (to_string 0) ++ ", " ++ ...`
  - **Working equivalent:** `mml/samples/mem/to_string_complex.mml` — same logic with
    direct nested `concat` calls; `mml/samples/concat-fn.mml` — same logic using `fn join`
  ...
  - **Root cause (confirmed via IR):** The `__ownership_result` wrappers generated by
    `wrapWithFrees` (line 378) free ALL owned bindings visible in scope, including ones
    from **outer scopes** that haven't been consumed yet. The `borrowedScope` mechanism
    (line 827-828) marks inherited owned bindings as Borrowed to prevent this, but it
    fails for right-associative nesting where the right-hand arg to a curried function
    is itself an allocating expression.
  - **IR evidence** (`ConcatOp-x86_64-apple-macosx.ll:110`):
    - `%0` (to_string 0) freed **6 times** — only used once
    - `%1` (to_string 123) freed **3 times** — only used once
    - `%8` (first `++` result) freed **before** next `++` consumes it
    - ConcatFn IR: each value freed exactly once, only after last use
  - **Trigger condition:** Right-associative operator chaining (`a ++ b ++ c` → `a ++ (b ++ c)`)
    puts allocating expressions in the **arg** position of curried App. This creates nested
    temp wrappers. During re-analysis (line 834), the inner `App(Lambda, arg)` CPS handler
    sees owned bindings from outer scopes and adds `wrapWithFrees` calls for them.
  - **Likely fix area:** `OwnershipAnalyzer` lines 823-840 — the `borrowedScope` construction
    and/or the `isTerminalBody` / `bindingsToFree` logic at lines 627-654. The scope passed
    to re-analysis must correctly exclude outer-scope owned bindings from the free list at
    every nesting level, not just the first.
