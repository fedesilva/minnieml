...
Report: 
 "In the AST, the synthetic bindings ($p0, __tmp_0, __tmp_result) all have resolvedId: None and empty candidateIds. The IR came out fine so codegen clearly handles them, but you might want to resolve those during name resolution for consistency — it'd make the AST self-contained and catch bugs earlier if you ever do AST-level transformations or diagnostics that depend on resolved IDs."

 source: mml/samples/partial-app-example.mml
 look at the ast generated.
